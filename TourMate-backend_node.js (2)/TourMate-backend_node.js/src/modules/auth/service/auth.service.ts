...
class authService {
    ...
    verifyResetCode = async (req: Request, res: Response) => {
        ...
        return res.json(successResponse("OTP verified successfully", 200));
    };
    getLoggedInUser = async (req: IRequest, res: Response) => {
        const user = req.loggedInUser!.user;
        const currentUser = await this.userRepo.findDocumentById(
            user._id,
            "-password -otps"
        );
        if (!currentUser) throw new badRequestException("User not found");
        return res.json(successResponse("User fetched successfully", 200, currentUser));
    };
