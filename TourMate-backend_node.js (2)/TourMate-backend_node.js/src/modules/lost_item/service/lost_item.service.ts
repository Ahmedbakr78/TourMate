...
class LostItemService {
    ...
    reportFoundItem = async (req: IRequest, res: Response) => {
        const user = req.loggedInUser!.user;
        const { id } = req.params as { id: string };
        if (!mongoose.isValidObjectId(id)) throw new badRequestException("Invalid lost item id");
        const lostItem = await this.lostItemRepo.findDocumentById(id);
        if (!lostItem) throw new badRequestException("Lost item not found");
        if (lostItem.status !== lostItemStatusEnum.PENDING) throw new badRequestException("Only pending items can be marked as found");
        const updated = await this.lostItemRepo.findDocumentByIdAndUpdate(id, { $set: { status: lostItemStatusEnum.FOUND } }, { new: true });
        await notificationService.createNotification({
            senderId: user._id.toString(),
            receiverId: lostItem.userId.toString(),
            title: "Lost Item Found",
        ...
        sendNotification(lostItem.userId.toString(), { title: "Lost Item Found", message: `Your lost item "${lostItem.title}" has been marked as found.` });
        return res.json(successResponse("Lost item marked as found", 200, updated));
    };
    closeLostItem = async (req: IRequest, res: Response) => {
        ...
        const lostItem = await this.lostItemRepo.findDocumentById(id);
        if (!lostItem) throw new badRequestException("Lost item not found");
        if (lostItem.status !== lostItemStatusEnum.FOUND) throw new badRequestException("Only found items can be closed");
        const updated = await this.lostItemRepo.findDocumentByIdAndUpdate(id, { $set: { status: lostItemStatusEnum.CLOSED } }, { new: true });
        await notificationService.createNotification({
            senderId: user._id.toString(),
            receiverId: lostItem.userId.toString(),
            title: "Lost Item Closed",
            message: `Your lost item "${lostItem.title}" has been closed.`
        });
        sendNotification(lostItem.userId.toString(), { title: "Lost Item Closed", message: `Your lost item "${lostItem.title}" has been closed.` });
        return res.json(successResponse("Lost item closed", 200, updated));
    };
    reopenLostItem = async (req: IRequest, res: Response) => {
        ...
        const lostItem = await this.lostItemRepo.findDocumentById(id);
        if (!lostItem) throw new badRequestException("Lost item not found");
        if (lostItem.status !== lostItemStatusEnum.CLOSED) throw new badRequestException("Only closed items can be reopened");
        const updated = await this.lostItemRepo.findDocumentByIdAndUpdate(id, { $set: { status: lostItemStatusEnum.PENDING } }, { new: true });
        await notificationService.createNotification({
            senderId: user._id.toString(),
            receiverId: lostItem.userId.toString(),
            title: "Lost Item Reopened",
            message: `Your lost item "${lostItem.title}" has been reopened.`
        });
        sendNotification(lostItem.userId.toString(), { title: "Lost Item Reopened", message: `Your lost item "${lostItem.title}" has been reopened.` });
        return res.json(successResponse("Lost item reopened", 200, updated));
    };
}
