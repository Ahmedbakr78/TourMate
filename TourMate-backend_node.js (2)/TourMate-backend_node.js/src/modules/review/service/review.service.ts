...
class reviewService {
    ...
    getGuideReviews = async (req: Request, res: Response) => {
        ...
        const reviews = await this.reviewRepo.findDocuments(
            ...
            {
                populate: [
                    { path: "touristId", select: "name profileImage" },
                    { path: "tripId" },
                    { path: "placeId", select: "name city" }
                ],
                sort: { createdAt: -1 }
            }
        );
        return res.json(successResponse("Guide reviews fetched successfully", 200, reviews));
    };
    getDriverReviews = async (req: Request, res: Response) => {
        ...
        return res.json(successResponse("Driver reviews fetched successfully", 200, reviews));
    };
    getMyReviews = async (req: IRequest, res: Response) => {
        const user = req.loggedInUser!.user;
        const reviews = await this.reviewRepo.findDocuments(
            { touristId: user._id },
            {},
        ...
        return res.json(successResponse("My reviews fetched successfully", 200, reviews));
    };
}
