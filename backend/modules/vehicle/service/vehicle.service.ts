...
class vehicleService {
    ...
    uploadVehicleImages = async (req: IRequest, res: Response) => {
        const { id } = req.params as { id: string };
        const files = req.files as Express.Multer.File[];
        const vehicle = await this.vehicleRepo.findDocumentById(id);
        if (!vehicle) throw new badRequestException("Vehicle not found");
        ...
        for (const file of files) {
            const uploadResult = await uploadFileToCloudinary(file.path, { folder: "vehicle_images" });
            await fs.unlink(file.path);
            uploadedImages.push({ secure_url: uploadResult.secure_url, public_id: uploadResult.public_id });
        }
        const updatedVehicle = await this.vehicleRepo.findDocumentByIdAndUpdate(
            id,
            { $push: { carImages: { $each: uploadedImages } } },
            { new: true, populate: { path: "driverId", populate: { path: "userId", select: "-password" } } }
        );
        return res.json(successResponse("Images uploaded successfully", 200, updatedVehicle));
    };
    deleteVehicleImage = async (req: IRequest, res: Response) => {
        ...
        return res.json(successResponse("Image deleted successfully", 200, updatedVehicle));
    };
}
