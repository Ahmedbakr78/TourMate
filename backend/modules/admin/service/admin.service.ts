...
import { pagination } from "../../../utils/pagination/pagination.utils.js";
...
class adminService {
    ...
    getAllUsers = async (req: IRequest, res: Response) => {
    ...
    getPendingGuides = async (req: IRequest, res: Response) => {
        ...
        return res.json(successResponse("Pending guides fetched successfully", 200, guides));
    };
    getPendingDrivers = async (req: IRequest, res: Response) => {
        ...
        return res.json(successResponse("Pending drivers fetched successfully", 200, drivers));
    };
    ...
    getReports = async (req: IRequest, res: Response) => {

        const [
            totalTourists, totalDrivers, totalGuides, totalAdmins,
            totalPendingTrips, totalActiveTrips, totalCompletedTrips, totalCancelledTrips,
            totalPendingLostItems, totalFoundLostItems, totalClosedLostItems
        ] = await Promise.all([

            this.userRepo.countDocuments({ role: roleEnum.TOURIST }),
            this.userRepo.countDocuments({ role: roleEnum.DRIVER }),
            this.userRepo.countDocuments({ role: roleEnum.GUIDE }),
            this.userRepo.countDocuments({ role: roleEnum.ADMIN }),

            this.tripRepo.countDocuments({ status: tripStatusEnum.PENDING }),
            this.tripRepo.countDocuments({ status: tripStatusEnum.ACTIVE }),
            this.tripRepo.countDocuments({ status: tripStatusEnum.COMPLETED }),
            this.tripRepo.countDocuments({ status: tripStatusEnum.CANCELLED }),
            ...
            this.lostItemRepo.countDocuments({ status: lostItemStatusEnum.PENDING }),
            this.lostItemRepo.countDocuments({ status: lostItemStatusEnum.FOUND }),
            this.lostItemRepo.countDocuments({ status: lostItemStatusEnum.CLOSED })
        ]);

        return res.json(successResponse("Reports fetched successfully", 200, {
            users: {
            ...
            lostItems: {
                ...
                found: totalFoundLostItems,
                closed: totalClosedLostItems
            }
        }));
    };
