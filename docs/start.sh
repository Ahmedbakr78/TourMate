...
test_endpoint "GET  /static JS           " GET  "/main.727be5977f11286f.js" 200
test_endpoint "POST /auth/signin         " POST "/auth/signin"              500
test_endpoint "POST /auth/signup         " POST "/auth/signup"              500
...
test_endpoint "GET  /admin/reports       " GET  "/admin/reports"            401
test_endpoint "GET  /trip/shared         " GET  "/trip/shared"              401
test_endpoint "GET  /notif/unread        " GET  "/notifications/unread-count" 401
test_endpoint "GET  /review/guide/:id    " GET  "/review/guide/fake"        401
...
if $TUNNEL; then
    ...
    if command -v cloudflared >/dev/null 2>&1; then
        ...
        for attempt in 1 2 3; do
            ...
            if [ -n "$URL" ]; then
                HOST=$(echo "$URL" | sed 's|https://||'); sleep 10
                if nslookup "$HOST" 2>/dev/null | grep -q "Name:"; then
            ...
            if grep -q "429\|Too Many" "$CLOUDFLARE_LOG" 2>/dev/null; then
                echo -e "  ${YELLOW}  Cloudflare rate limited, trying localtunnel...${NC}"; URL=""; break
            fi
            sleep 2
        done
    fi
    if [ -z "$URL" ] || ! nslookup "$(echo "$URL" | sed 's|https://||')" 2>/dev/null | grep -q "Name:"; then
        echo -e "  ${YELLOW}  Using localtunnel instead of Cloudflare...${NC}"
        LT_LOG=/tmp/localtunnel.log
        kill $LT_PID 2>/dev/null || true
        npx -y localtunnel --port $PORT > "$LT_LOG" 2>&1 &
        LT_PID=$!
        for i in $(seq 1 20); do
            URL=$(grep -oP 'https?://[a-zA-Z0-9-]+\.loca\.lt' "$LT_LOG" 2>/dev/null | head -1)
            if [ -n "$URL" ]; then break; fi
            sleep 1
        done
        echo -e "  ${GREEN}  Localtunnel: ${BOLD}$URL${NC}"
        echo ""
    fi
fi
